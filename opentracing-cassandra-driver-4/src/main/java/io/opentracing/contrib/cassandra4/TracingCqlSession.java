...
public class TracingCqlSession implements CqlSession {
    ...
    public ResultSet execute(@NonNull Statement<?> statement) {
        ...
        final Span span = spanBuilder(query).start();
    ...
    private SpanBuilder spanBuilder(String query) {
        final SpanBuilder b = tracer.buildSpan(QueryMethodTableSpanName.INSTANCE.querySpanName(query))
                .withTag(Tags.SPAN_KIND.getKey(), Tags.SPAN_KIND_CLIENT)
                .withTag(Tags.COMPONENT.getKey(), COMPONENT_NAME)
                .withTag(Tags.DB_TYPE.getKey(), "cassandra")
                .withTag(Tags.DB_STATEMENT, query);

        session.getKeyspace().ifPresent(cqlIdentifier -> b.withTag(Tags.DB_INSTANCE.getKey(), cqlIdentifier.asInternal()));
        return b;
    }
}
